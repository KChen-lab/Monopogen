...
study_coord <-data.frame("PopID"="study", "indivID"=study[,1], "PC1"=study[,7], "PC2"=study[,8],"PC3"=study[,9])
...
p <- ggplot(dt_plt, aes(x=PC1, y=PC3, color=PopID)) + geom_point(size=4,alpha=0.1) +  scale_color_manual(values=c(vega_20[1:7],"black")) + theme_bw() + theme( panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), text = element_text(size = 16)) + guides(colour=guide_legend(override.aes = list(alpha=0.9)))

p <- p + geom_point(data=dt_plt[dt_plt$PopID=="study",],  col="black",size=4, shape=4)

p2 <- p
pdf(file=paste0(args[3],".pdf"),width=6.5, height=4.5)
print(p1)
print(p2)
dev.off()
